...
In modern Security Operations Centers (SOCs), human analysts are generally overwhelmed by the volume of daily alerts. Because of this workload, sometimes things can slip through, leaving networks vulnerable. 
...
Micro SOC Gym forces the agent into a proactive role. It challenges the agent's ability to:
...
The `MicroSocGymAction` requires specifying exactly one `tool` per step. Available tools are categorised into Investigative and Remediation tools:
...
After a lot of experimentation, testing and optimisations, we settled on this reward structure:
...
| **-0.25** | `ACTION_TO_STALL_PENALTY`                      | Given when the agent tries re-investigation after executing all possible remediation actions.<br><br>Introduced when we saw the agent re-read logfiles after trying out all the remediation tools at least once. This is basically the agent stalling as it already has the information it needs, but is executing the remediation actions incorrectly.                                                                                                                                            |
| **-0.30** | `UNWARRANTED_ACTION_REPEAT_PENALTY`            | Given when the agent unnecessarily repeats actions to game the system.<br><br>Introduced when we saw the agent read the same logs consecutively to continuously gain rewards. When a penalty on consecutive repeat was implemented, the agent started reading logfiles in an alternate pattern (like `read_access_log`, `read_auth_log`, `read_access_log`...). Agents could also re-block the same IP again and again, to continuously gain rewards. <br><br>This penalty prevents the agent from exploiting such actions. |
...
We ran the `Qwen/Qwen2.5-72B-Instruct` model through all three scenarios using our inference script. Below are the results:
